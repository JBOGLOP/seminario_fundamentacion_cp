# Sesión 01 · Antecedentes históricos y fundamentos filosóficos

**viernes 21 de agosto de 2026** · Presencial · 3 horas · Primer corte
Unidad 1 — Contexto histórico-filosófico y lineamientos sociopolíticos

> Nota de trabajo. La ficha pública es [`index.html`](index.html); este archivo es para el
> docente y no pretende ser bonito.

---

## De dónde sale

Condensa **dos sesiones** de *Contexto Histórico y Legal de los Cuidados Paliativos* (37542001,
2026-I), que sumaban 3 h 30 min de clase:

| Origen | Formato en 2026-I | Duración |
|---|---|---|
| [`s01-contexto-historico`](https://jboglop.github.io/Contexto_Historico_Legal_CP/sesiones/s01-contexto-historico/) | Presencial, 6 feb 2026 | 2 h |
| [`s02-genealogia-del-cuidado`](https://jboglop.github.io/Contexto_Historico_Legal_CP/sesiones/s02-genealogia-del-cuidado/) | Sincrónica, 20 feb 2026 | 90 min |

Fuentes de segundo orden, en el taller (no en el repositorio):
`1. Contexto Histórico y Legal CP/1. Clase 1 Contexto historico/` y `/2. Clase 2. Genealogia del
cuidado/` — `Clase1_contextohistorico.pptx`, `genealogia del CP.pptx`,
`Encuentro_Sincronico_2_Genealogia_del_Cuidado_v2.docx`, `historia de CP.pdf`, `fr_sanbernardo.pdf`.
...
| # | Pregunta | Qué mide |
|---|---|---|
| P1 | ¿Cuándo llegaron los CP a donde usted trabaja? | Búsqueda real y **reporte del resultado negativo**. La ausencia de registro es el hallazgo |
...
### El guion de las tres horas

| Hora | Qué pasa |
|---|---|
| 8:00–8:20 | Encuadre del seminario. **Solo esta vez** |
| 8:20–9:20 | P1 y P2 proyectadas: el mapa de los territorios del grupo y sus huecos |
| 9:30–10:20 | P3 y P4: del juicio al dato; qué deja invisible cada indicador |
| 10:20–10:50 | P5 en plenaria: se depuran las preguntas de investigación del grupo |
| 10:50–11:00 | Cierre y puente a la sesión 2 |
...
- [x] Enlazar desde la portada del seminario
- [x] Pasar `node scripts/verificar.js` en verde
- [ ] 🔴 **Instalar el motor** (`scripts/apps-script/INSTALACION.md`) y pegar `ENDPOINT` y `TOKEN`
      en `preparacion.html`. **Sin esto no se recibe ninguna entrega.**
- [ ] 🔴 **Cargar el roster** con los correos institucionales del grupo, y ejecutar
      `asignarSeudonimos()`
- [ ] 🔴 **Abrir la ventana** en la pestaña `Config`: `S01`, cierre `2026-08-21 07:00`, `Activa = Sí`
- [ ] Publicar el enlace del entregable al grupo con tiempo suficiente para leer y escribir
- [ ] **Abrir en el navegador con el wifi apagado** y revisarla a 375 px (§7.5, no automatizable)
- [ ] Probar el **modo proyección** del tablero antes de compartir pantalla en clase
- [ ] Resolver los dos `[DATO POR VERIFICAR]` contra fuente primaria
- [ ] Definir la rúbrica con que se califican las cinco preguntas (la columna «Método de
      evaluación» del contenido programático está vacía — ver `docs/PROGRAMADOR-2026-II.md` §6.4)

## Pendientes propios

- **La carga declarada son ≈3 horas.** Es una estimación sin comprobar. El campo «minutos
  dedicados» del entregable existe para corregirla en la sesión 2 con datos reales.
- **P4 depende de que el Atlas esté accesible.** Es un PDF grande y en inglés; si el grupo
  tropieza, conviene alojar el extracto pertinente o dar la página exacta.
- No hay material de apoyo propio (imágenes, esquemas). Las presentaciones originales tienen
  imágenes de procedencia no documentada y **no se reproducen**. Si se quiere apoyo visual, hay
  que producirlo.
- El tablero solo tiene los títulos legibles de las preguntas de `S01` (constante `TITULOS`).
  Al construir S02 hay que añadir su bloque.
